...
export async function activationController (request: Request, response: Response, ) : Promise<Response<Status>> {
    try {
        const validationResult = z.object({
            activation: z.string().length(32, {message: 'please provide a valid profileActivationToken'}) }).safeParse(request.params)
        ...
        return response.json({
            status: 500, data: null, message: error.message})
       }
    }
